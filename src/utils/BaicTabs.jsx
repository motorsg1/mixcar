...
const BaicBrand = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const url = `${apiBaseUrl}wc/v3/products?brand=Baic&page=${currentPage}&acf_format=standard`;
      ...
      if (response.data.length === 0) {
        ...
        const newAcfData = await axios.get(`${apiBaseUrl}wc/v3/products?acf_format=standard&brand=Baic&page=${currentPage}`, {
          auth: apiAuth,
        });
  ...
  const renderProduct = (product, index) => {
    ...
    return (
      <div key={`${product.id}-${index}`} className="col-12 col-md-4 mb-4">
