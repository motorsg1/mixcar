...
const MaxusBrand = () => {
  ...
  const fetchAcfData = async () => {
    try {
      const response = await axios.get(`${apiBaseUrl}wc/v3/products?acf_format=standard&brand=Maxus`, {
        auth: apiAuth,
      });
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const url = `${apiBaseUrl}wc/v3/products?brand=Maxus&page=${currentPage}&acf_format=standard`;
      ...
      if (response.data.length === 0) {
        ...
        const newAcfData = await axios.get(`${apiBaseUrl}wc/v3/products?acf_format=standard&brand=Maxus&page=${currentPage}`, {
          auth: apiAuth,
        });
