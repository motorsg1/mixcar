...
const RecentCar = () => {
  ...
  const fetchAcfData = async () => {
    const response = await fetchData(`wc/v3/products?acf_format=standard&lang=${router.locale}`);
    if (response) {
      setAcfData(response);
    } else {
      setAcfData([]);
  ...
  useEffect(() => {
    const fetchRecentProducts = async () => {
      const response = await fetchData(`wc/v3/products?orderby=date&order=desc&per_page=10&lang=${router.locale}`);
      if (response) {
    ...
    const fetchBrands = async () => {
      const response = await fetchData(`wc/v3/products/categories?lang=${router.locale}`);
      if (response) {
