...
const ProductFeatured = ({ productId }) => {
  ...
  const fetchAcfData = async () => {
    const response = await fetchData(`wc/v3/products?acf_format=standard`);
    setAcfData(response);
  };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const response = await fetchData('wc/v3/products');
      setProducts(response);
      setLoading(false);
    };

    const fetchBrands = async () => {
      const response = await fetchData('wc/v3/products/categories');
  ...
  useEffect(() => {
    if (products.length > 0 && brands.length > 0) {
      // Select a random brand from the available brands
      const randomBrand = brands[Math.floor(Math.random() * brands.length)];

      // Filter products by the selected random brand
      const randomBrandProducts = products.filter((product) => {
        return product.categories && product.categories.some(category => category.name === randomBrand.name);
      });

      // If the filtered products array is empty, use the original products array
      if (randomBrandProducts.length === 0) {
        setFilteredProducts(products.slice(0, 10));
      } else {
        setFilteredProducts(randomBrandProducts.slice(0, 10));
      }
    }
  }, [products, brands]);
    ...
    <div className="home5-featured-cars-section pt-10 mb-10">
      <div className="container">
              ...
              <div className="swiper-wrapper">
                {loading ? (
                  ...
                  Array(10).fill(0).map((_, index) => (
                    ...
                    // Once data is loaded, render the filtered products
                    filteredProducts && filteredProducts.length > 0 ? (
                        ...
                        <SwiperSlide key={product.id} className="swiper-slide">
                            ...
                            <div className="product-info">
                                ...
                                <p style={{ fontSize: '16px', fontWeight: '400', marginTop: '0px' }}>Category: {product.categories[0].name}</p>
                                <p style={{ fontSize: '20px', fontWeight: '600', marginTop: '0px' }}>Brand: {product.brands && product.brands.length > 0 ? product.brands[0].name : 'N/A'}</p>
                                ...
                                <Link href={`/car-details/${product.id}`} className="view-details-link">View Details</Link>
                                </div>
                            </div>
                            </div>
                        </SwiperSlide>
        ...
        <div className="row wow fadeInUp" data-wow-delay="300ms">
          ...
          <Link href="/brand-category" style={{ justifyContent: 'flex-end', marginTop: '15px', marginBottom: '20px' }}>Explore More<i className="bi bi-arrow-right" /></Link>
          </div>
        </div>
      </div>
    </div>
